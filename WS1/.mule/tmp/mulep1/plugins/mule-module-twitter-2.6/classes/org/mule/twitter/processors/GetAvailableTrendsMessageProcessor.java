...
/**
 * GetAvailableTrendsMessageProcessor invokes the {@link org.mule.twitter.TwitterConnector#getAvailableTrends(java.lang.Double, java.lang.Double)} method in {@link TwitterConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class GetAvailableTrendsMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(GetAvailableTrendsMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getAvailableTrends"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            _castedModuleObject = ((TwitterConnectorLifecycleAdapter) moduleObject);
        }
        try {
            Double _transformedLatitude = ((Double) evaluateAndTransform(_muleMessage, GetAvailableTrendsMessageProcessor.class.getDeclaredField("_latitudeType").getGenericType(), null, latitude));
            Double _transformedLongitude = ((Double) evaluateAndTransform(_muleMessage, GetAvailableTrendsMessageProcessor.class.getDeclaredField("_longitudeType").getGenericType(), null, longitude));
            retryCount.getAndIncrement();
            Object resultPayload;
            resultPayload = _castedModuleObject.getAvailableTrends(_transformedLatitude, _transformedLongitude);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("getAvailableTrends"), event, e);
        }
    }
