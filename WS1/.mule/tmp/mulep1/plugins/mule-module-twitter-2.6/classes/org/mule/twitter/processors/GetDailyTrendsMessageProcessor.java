...
/**
 * GetDailyTrendsMessageProcessor invokes the {@link org.mule.twitter.TwitterConnector#getDailyTrends(java.util.Date, boolean)} method in {@link TwitterConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class GetDailyTrendsMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(GetDailyTrendsMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getDailyTrends"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            _castedModuleObject = ((TwitterConnectorLifecycleAdapter) moduleObject);
        }
        try {
            Date _transformedDate = ((Date) evaluateAndTransform(_muleMessage, GetDailyTrendsMessageProcessor.class.getDeclaredField("_dateType").getGenericType(), null, date));
            Boolean _transformedExcludeHashTags = ((Boolean) evaluateAndTransform(_muleMessage, GetDailyTrendsMessageProcessor.class.getDeclaredField("_excludeHashTagsType").getGenericType(), null, excludeHashTags));
            retryCount.getAndIncrement();
            Object resultPayload;
            resultPayload = _castedModuleObject.getDailyTrends(_transformedDate, _transformedExcludeHashTags);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("getDailyTrends"), event, e);
        }
    }
