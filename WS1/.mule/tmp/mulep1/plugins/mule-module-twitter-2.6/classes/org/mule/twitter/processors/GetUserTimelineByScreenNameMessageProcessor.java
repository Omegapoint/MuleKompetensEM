...
/**
 * GetUserTimelineByScreenNameMessageProcessor invokes the {@link org.mule.twitter.TwitterConnector#getUserTimelineByScreenName(java.lang.String, int, int, int)} method in {@link TwitterConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class GetUserTimelineByScreenNameMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private String _screenNameType;
    private Object page;
    ...
    private static Logger logger = LoggerFactory.getLogger(GetUserTimelineByScreenNameMessageProcessor.class);
    /**
     * Module object
     * 
     */
     ...
     * @param value Value to set
     */
    public void setScreenName(Object value) {
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getUserTimelineByScreenName"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            _castedModuleObject = ((TwitterConnectorLifecycleAdapter) moduleObject);
        }
        try {
            String _transformedScreenName = ((String) evaluateAndTransform(_muleMessage, GetUserTimelineByScreenNameMessageProcessor.class.getDeclaredField("_screenNameType").getGenericType(), null, screenName));
            Integer _transformedPage = ((Integer) evaluateAndTransform(_muleMessage, GetUserTimelineByScreenNameMessageProcessor.class.getDeclaredField("_pageType").getGenericType(), null, page));
            Integer _transformedCount = ((Integer) evaluateAndTransform(_muleMessage, GetUserTimelineByScreenNameMessageProcessor.class.getDeclaredField("_countType").getGenericType(), null, count));
            Integer _transformedSinceId = ((Integer) evaluateAndTransform(_muleMessage, GetUserTimelineByScreenNameMessageProcessor.class.getDeclaredField("_sinceIdType").getGenericType(), null, sinceId));
            retryCount.getAndIncrement();
            Object resultPayload;
            resultPayload = _castedModuleObject.getUserTimelineByScreenName(_transformedScreenName, _transformedPage, _transformedCount, _transformedSinceId);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("getUserTimelineByScreenName"), event, e);
        }
    }
