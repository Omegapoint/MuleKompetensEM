...
/**
 * GetUserTimelineByUserIdMessageProcessor invokes the {@link org.mule.twitter.TwitterConnector#getUserTimelineByUserId(long, int, int, int)} method in {@link TwitterConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class GetUserTimelineByUserIdMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(GetUserTimelineByUserIdMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getUserTimelineByUserId"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            _castedModuleObject = ((TwitterConnectorLifecycleAdapter) moduleObject);
        }
        try {
            Long _transformedUserId = ((Long) evaluateAndTransform(_muleMessage, GetUserTimelineByUserIdMessageProcessor.class.getDeclaredField("_userIdType").getGenericType(), null, userId));
            Integer _transformedPage = ((Integer) evaluateAndTransform(_muleMessage, GetUserTimelineByUserIdMessageProcessor.class.getDeclaredField("_pageType").getGenericType(), null, page));
            Integer _transformedCount = ((Integer) evaluateAndTransform(_muleMessage, GetUserTimelineByUserIdMessageProcessor.class.getDeclaredField("_countType").getGenericType(), null, count));
            Integer _transformedSinceId = ((Integer) evaluateAndTransform(_muleMessage, GetUserTimelineByUserIdMessageProcessor.class.getDeclaredField("_sinceIdType").getGenericType(), null, sinceId));
            retryCount.getAndIncrement();
            Object resultPayload;
            resultPayload = _castedModuleObject.getUserTimelineByUserId(_transformedUserId, _transformedPage, _transformedCount, _transformedSinceId);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("getUserTimelineByUserId"), event, e);
        }
    }
