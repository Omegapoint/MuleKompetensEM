...
/**
 * GetWeeklyTrendsMessageProcessor invokes the {@link org.mule.twitter.TwitterConnector#getWeeklyTrends(java.util.Date, boolean)} method in {@link TwitterConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class GetWeeklyTrendsMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{

    private Object date;
    private Date _dateType;
    private Object excludeHashTags;
    private boolean _excludeHashTagsType;
    private static Logger logger = LoggerFactory.getLogger(GetWeeklyTrendsMessageProcessor.class);
    /**
     * Module object
     * 
     */
    ...
    /**
     * Sets excludeHashTags
     * 
     * @param value Value to set
     */
    public void setExcludeHashTags(Object value) {
        this.excludeHashTags = value;
    }
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("getWeeklyTrends"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            _castedModuleObject = ((TwitterConnectorLifecycleAdapter) moduleObject);
        }
        try {
            Date _transformedDate = ((Date) evaluateAndTransform(_muleMessage, GetWeeklyTrendsMessageProcessor.class.getDeclaredField("_dateType").getGenericType(), null, date));
            Boolean _transformedExcludeHashTags = ((Boolean) evaluateAndTransform(_muleMessage, GetWeeklyTrendsMessageProcessor.class.getDeclaredField("_excludeHashTagsType").getGenericType(), null, excludeHashTags));
            retryCount.getAndIncrement();
            Object resultPayload;
            resultPayload = _castedModuleObject.getWeeklyTrends(_transformedDate, _transformedExcludeHashTags);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("getWeeklyTrends"), event, e);
        }
    }
