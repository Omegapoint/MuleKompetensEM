    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private String _callbackUrlType;
    private static Logger logger = LoggerFactory.getLogger(RequestAuthorizationMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("requestAuthorization"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            _castedModuleObject = ((TwitterConnectorLifecycleAdapter) moduleObject);
        }
        try {
            String _transformedCallbackUrl = ((String) evaluateAndTransform(_muleMessage, RequestAuthorizationMessageProcessor.class.getDeclaredField("_callbackUrlType").getGenericType(), null, callbackUrl));
            retryCount.getAndIncrement();
            Object resultPayload;
            resultPayload = _castedModuleObject.requestAuthorization(_transformedCallbackUrl);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("requestAuthorization"), event, e);
        }
    }
