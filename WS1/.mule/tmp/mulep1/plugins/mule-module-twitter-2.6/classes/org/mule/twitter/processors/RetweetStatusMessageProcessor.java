...
/**
 * RetweetStatusMessageProcessor invokes the {@link org.mule.twitter.TwitterConnector#retweetStatus(long)} method in {@link TwitterConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class RetweetStatusMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(RetweetStatusMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("retweetStatus"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            _castedModuleObject = ((TwitterConnectorLifecycleAdapter) moduleObject);
        }
        try {
            Long _transformedStatusId = ((Long) evaluateAndTransform(_muleMessage, RetweetStatusMessageProcessor.class.getDeclaredField("_statusIdType").getGenericType(), null, statusId));
            retryCount.getAndIncrement();
            Object resultPayload;
            resultPayload = _castedModuleObject.retweetStatus(_transformedStatusId);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("retweetStatus"), event, e);
        }
    }
