...
/**
 * SearchPlacesMessageProcessor invokes the {@link org.mule.twitter.TwitterConnector#searchPlaces(java.lang.Double, java.lang.Double, java.lang.String)} method in {@link TwitterConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class SearchPlacesMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(SearchPlacesMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("searchPlaces"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            _castedModuleObject = ((TwitterConnectorLifecycleAdapter) moduleObject);
        }
        try {
            Double _transformedLatitude = ((Double) evaluateAndTransform(_muleMessage, SearchPlacesMessageProcessor.class.getDeclaredField("_latitudeType").getGenericType(), null, latitude));
            Double _transformedLongitude = ((Double) evaluateAndTransform(_muleMessage, SearchPlacesMessageProcessor.class.getDeclaredField("_longitudeType").getGenericType(), null, longitude));
            String _transformedIp = ((String) evaluateAndTransform(_muleMessage, SearchPlacesMessageProcessor.class.getDeclaredField("_ipType").getGenericType(), null, ip));
            retryCount.getAndIncrement();
            Object resultPayload;
            resultPayload = _castedModuleObject.searchPlaces(_transformedLatitude, _transformedLongitude, _transformedIp);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("searchPlaces"), event, e);
        }
    }
