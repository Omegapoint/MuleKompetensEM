...
/**
 * SendDirectMessageByUserIdMessageProcessor invokes the {@link org.mule.twitter.TwitterConnector#sendDirectMessageByUserId(long, java.lang.String)} method in {@link TwitterConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class SendDirectMessageByUserIdMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(SendDirectMessageByUserIdMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("sendDirectMessageByUserId"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            _castedModuleObject = ((TwitterConnectorLifecycleAdapter) moduleObject);
        }
        try {
            Long _transformedUserId = ((Long) evaluateAndTransform(_muleMessage, SendDirectMessageByUserIdMessageProcessor.class.getDeclaredField("_userIdType").getGenericType(), null, userId));
            String _transformedMessage = ((String) evaluateAndTransform(_muleMessage, SendDirectMessageByUserIdMessageProcessor.class.getDeclaredField("_messageType").getGenericType(), null, message));
            retryCount.getAndIncrement();
            Object resultPayload;
            resultPayload = _castedModuleObject.sendDirectMessageByUserId(_transformedUserId, _transformedMessage);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("sendDirectMessageByUserId"), event, e);
        }
    }
