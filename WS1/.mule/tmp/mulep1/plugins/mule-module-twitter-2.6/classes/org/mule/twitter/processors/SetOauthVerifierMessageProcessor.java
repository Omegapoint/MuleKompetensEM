    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private Object oauthVerifier;
    private String _oauthVerifierType;
    private static Logger logger = LoggerFactory.getLogger(SetOauthVerifierMessageProcessor.class);
    /**
     * Module object
     * 
     */
     ...
     * @param value Value to set
     */
    public void setOauthVerifier(Object value) {
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("setOauthVerifier"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            _castedModuleObject = ((TwitterConnectorLifecycleAdapter) moduleObject);
        }
        try {
            String _transformedOauthVerifier = ((String) evaluateAndTransform(_muleMessage, SetOauthVerifierMessageProcessor.class.getDeclaredField("_oauthVerifierType").getGenericType(), null, oauthVerifier));
            retryCount.getAndIncrement();
            _castedModuleObject.setOauthVerifier(_transformedOauthVerifier);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
            throw new MessagingException(CoreMessages.failedToInvoke("setOauthVerifier"), event, e);
        }
    }
