    ...
    implements Runnable, SourceCallback, FlowConstructAware, MuleContextAware, Initialisable, Startable, Stoppable, MessageSource
{
    ...
    public void run() {
        ...
        try {
            ...
            List<String> transformedKeywords = null;
            if (keywords!= null) {
                if (!List.class.isAssignableFrom(keywords.getClass())) {
                    DataType source;
                    DataType target;
                    source = DataTypeFactory.create(keywords.getClass());
                    target = DataTypeFactory.create(List.class);
                    Transformer t;
                    t = muleContext.getRegistry().lookupTransformer(source, target);
                    transformedKeywords = ((List<String> ) t.transform(keywords));
                } else {
                    transformedKeywords = ((List<String> ) keywords);
                }
            }
            castedModuleObject.filteredStream(transformedCount, transformedUserIds, transformedKeywords, this);
