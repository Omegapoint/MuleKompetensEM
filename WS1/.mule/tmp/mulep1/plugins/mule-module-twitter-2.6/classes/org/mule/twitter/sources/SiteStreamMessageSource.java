    ...
    implements Runnable, SourceCallback, FlowConstructAware, MuleContextAware, Initialisable, Startable, Stoppable, MessageSource
{
    ...
    private Object withFollowings;
    private boolean _withFollowingsType;
    /**
     * Module object
     * 
     */
        ...
        try {
            if (moduleObject instanceof String) {
                ...
                if (castedModuleObject == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("siteStream"), ((MuleEvent) null), new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
                }
            } else {
                castedModuleObject = ((TwitterConnectorLifecycleAdapter) moduleObject);
            }
            List<Long> transformedUserIds = null;
            if (userIds!= null) {
                if (!List.class.isAssignableFrom(userIds.getClass())) {
                    DataType source;
                    DataType target;
                    source = DataTypeFactory.create(userIds.getClass());
                    target = DataTypeFactory.create(List.class);
                    Transformer t;
                    t = muleContext.getRegistry().lookupTransformer(source, target);
                    transformedUserIds = ((List<Long> ) t.transform(userIds));
                } else {
                    transformedUserIds = ((List<Long> ) userIds);
                }
            }
            Boolean transformedWithFollowings = null;
            if (withFollowings!= null) {
                if (!Boolean.class.isAssignableFrom(withFollowings.getClass())) {
                    DataType source;
                    DataType target;
                    source = DataTypeFactory.create(withFollowings.getClass());
                    target = DataTypeFactory.create(Boolean.class);
                    Transformer t;
                    t = muleContext.getRegistry().lookupTransformer(source, target);
                    transformedWithFollowings = ((Boolean) t.transform(withFollowings));
                } else {
                    transformedWithFollowings = ((Boolean) withFollowings);
                }
            }
            castedModuleObject.siteStream(transformedUserIds, transformedWithFollowings, this);
